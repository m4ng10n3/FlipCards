...
assert.equal(previousTaskID(history,'specialista'),undefined)
assert.equal(previousTaskID(history,'rapido'),undefined)
assert.equal(buildHandoff(history,{subagent_type:'specialista',prompt:'Continue'}).task_id,undefined)
assert.equal(buildHandoff(history,{subagent_type:'specialista',task_id:'ses_123ABC',prompt:'Verify missing criterion'}).task_id,'ses_123ABC')

...
c=clear_background(root/'_Sources/v2_cabinet_ink_generated.png',root/'02_Machine/cabinet_illustrated.png',[(440,430),(884,430),(1350,430)])
clear_background(root/'_Sources/v2_button_ink_generated.png',root/'04_Controls/mushroom_idle.png')
clear_background(root/'_Sources/v2_lever_ink_generated.png',root/'04_Controls/lever_rest.png')
clear_background(root/'_Sources/v2_reel_generated.png',root/'02_Machine/reel_drum_blank.png')
...
for folder in ['02_Machine','03_Table','04_Controls','05_Lamps','06_UI']:
    for p in (root/folder).glob('*.png'):
        im=Image.open(p);files.append({'file':str(p.relative_to(root)).replace('\\','/'),'size':list(im.size),'mode':im.mode,'alpha':im.getchannel('A').getextrema() if im.mode=='RGBA' else None})
(root/'07_Spec/asset_inventory.json').write_text(json.dumps(files,indent=2))

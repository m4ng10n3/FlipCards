...
# This script lives with the studies; the table the game mounts and the spec
# live in the active style. Intermediate layers stay with the studies.
root=Path(__file__).resolve().parents[1]
active=next(p for p in root.parents if p.name=='Assets')/'Graphics/1_NeonMonte_Attivo/SceneKit_v2'
source=active.parent/'Cards/_Final/Back/Template/back_artwork_overlay.png'
...
for name,base,out in [('empty','plain_empty.png',active/'03_Table/table_original_medallion_empty.png'),('occupied','plain_occupied.png',root/'01_Layout/layout_original_medallion.png')]:
...
(active/'07_Spec/original_medallion_projection.json').write_text(json.dumps(report,indent=2))

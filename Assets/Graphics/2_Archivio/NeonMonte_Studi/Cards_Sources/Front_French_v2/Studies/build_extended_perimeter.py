...
for x in range(int(D[0])+16,int(D[0])+77,2):
    if x not in boundary:continue
    C=np.array((float(x),float(boundary[x])))
    Bs=[p for p in contour[:contour.index(tuple(map(int,C)))] if p[0]>W/2 and 350<=p[1]<=600]
    if not Bs:continue
    B=min(Bs,key=lambda p:abs(p[1]-(2*C[1]-D[1])))
    for reach in np.linspace(.50,2.10,17):
        for rise in (24,36,48,60,72,84,96,108,120,132,144,156,168):
            for indent in (64,80,96,112,128,144,160,176,192,208):
                U=np.array((C[0]-reach*(D[1]-C[1]),C[1]-rise))
                V=np.array((D[0]-indent,D[1]-.50*(D[1]-C[1])))
                arc=cubic(C,U,V,D)
                ...
                # User constraints lead: equal vertical gaps and C close to D
                # in X; regularize waist and upper return for readability.
                gap_error=(C[1]-B[1])-(D[1]-C[1])
                score=(gap_error/8)**2+((C[0]-D[0]-32)/24)**2
                score+=((neck_width/blade_width-.25)/.06)**2
                score+=.12*((rise/(D[1]-C[1])-.22)/.12)**2+.05*((reach-.85)/.35)**2
                feasible+=1
...
previous=json.loads((ROOT/'F_VARIABLE_CURVE_GEOMETRY.json').read_text(encoding='utf-8'))
old_b=tuple(previous['B']);old_c=tuple(map(int,previous['C']))
old_bc=contour[contour.index(old_b):contour.index(old_c)+1]
old_arc=cubic(np.array(old_c),np.array(previous['U']),np.array(previous['V']),D).tolist()
old_outline=[A]+old_bc+old_arc[1:]+[mirror(D)]+[mirror(p) for p in reversed(old_arc[:-1])]+[mirror(p) for p in reversed(old_bc[:-1])]
old_clip=Image.new('L',base.size);ImageDraw.Draw(old_clip).polygon(old_outline,fill=255)
old_kept=ImageChops.multiply(base,old_clip);old_kept=ImageChops.darker(old_kept,old_kept.transpose(Image.Transpose.FLIP_LEFT_RIGHT))
source_area=np.count_nonzero(pixels)
area_before=np.count_nonzero(np.asarray(old_kept))/source_area
area_after=np.count_nonzero(np.asarray(kept))/source_area
def path_length(path):return sum(math.dist(a,b) for a,b in zip(path,path[1:]))
perimeter_before=2*path_length(old_bc)/path_length(contour)
perimeter_after=2*path_length(right_curve)/path_length(contour)
assert area_after>area_before and perimeter_after>perimeter_before
...
sprite.alpha_composite(thumb,((256-thumb.width)//2,(256-thumb.height)//2));sprite.save(ROOT/'attack_G_extended_perimeter.png')
...
(ROOT/'attack_G_extended_perimeter.svg').write_text(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}"><defs><clipPath id="shape"><path d="{cmd}"/></clipPath></defs><path fill="#192124" d="{cmd}"/><rect x="{W/2}" width="{W/2}" height="{H}" fill="#3d4b4d" clip-path="url(#shape)"/></svg>',encoding='utf-8')
...
txt(36,28,'G / MAGGIORE PERIMETRO DELLA PICCA',29)
txt(36,76,'B sale, C scende verso la verticale di D. Intervalli verticali B-C e C-D uguali.',18)
...
txt(646,638,'G / LAMA RISULTANTE',21);txt(1083,638,'CUORE / 88 x 88',21)
txt(645,674,f'{visible_w:.1f} x 104 nel template',17)
txt(645,704,f'Delta Y: B-C {C[1]-B[1]} / C-D {int(D[1]-C[1])}',17)
place(cropped,749,762,42,42/visible_w*104);place(heart,816,770,46,46)
txt(36,851,'A-B resta rettilineo; B-C conserva il contorno; C-D raccorda il gambo all\'angolo della picca.',18)
canvas.resize((1440,920),Image.Resampling.LANCZOS).save(ROOT/'G_EXTENDED_PERIMETER_CONSTRUCTION.png')
...
meta.update({'source_size':[W,H],'feasible_candidates':feasible,'template_scale':ratio,'template_ink_size':[visible_w,104],
             ...
             'old_B':[658,586],'old_C':[610,632],
             'vertical_gaps':[C[1]-B[1],int(D[1]-C[1])],'C_D_horizontal_gap':float(C[0]-D[0]),
             'retained_source_area_before_after':[area_before,area_after],
             'B_C_source_perimeter_fraction_before_after':[perimeter_before,perimeter_after],
             'objective':'(vertical_gap_difference/8)^2+((C.x-D.x-32)/24)^2+((neck/blade_width-.25)/.06)^2+.12*((rise/span-.22)/.12)^2+.05*((reach-.85)/.35)^2',
...
(ROOT/'G_EXTENDED_PERIMETER_GEOMETRY.json').write_text(json.dumps(meta,indent=2),encoding='utf-8')

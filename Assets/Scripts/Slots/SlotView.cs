...
public class SlotView : MonoBehaviour
{
    ...
    // tracking solo HP, niente pi� hint automatici su DMG/BLK
    ...
    void Awake()
    {
        ...
        // Preview editor-only: se non c'� istanza runtime, mostra i dati dell'inline
        PreviewFromInlineIfNoInstance();
        if (hintText != null) hintText.gameObject.SetActive(false);
    }

    /// <summary>
    /// Anteprima: mostra i dati statici se la carta � presente in scena ma senza istanza runtime.
    /// </summary>
    ...
    {
        if (instance == null) return "";
        string current = instance.side == Side.Fronte ? "▶ FRONTE" : "◀ RETRO";
        ...
        if (pattern != null && pattern.Length > 1)
        {
            ...
            string next = pattern[nextIdx] == Side.Fronte ? "▶" : "◀";
            current += $"  →{next}";
        }
        return current;
    }
